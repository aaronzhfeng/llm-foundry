...
if ddp and use_zero1:
    ...
    print(f"fused AdamW available: {fused_available}, but not used with ZeRO-1")
...
if init_from == 'resume' and checkpoint.get('optimizer') is not None:
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                # For ZeRO-1, consolidate optimizer state before saving
                if ddp and use_zero1:
                    optimizer.consolidate_state_dict(to=0)
                
                checkpoint = {
                    'model': raw_model.state_dict(),
                    'optimizer': optimizer.state_dict() if not (ddp and use_zero1) or ddp_rank == 0 else None,
