...
class ConfigurableGPT(nn.Module):
    ...
    def estimate_mfu_detailed(self, fwdbwd_per_iter, dt, device_type='cuda', num_gpus=1, use_sparse_specs=False):
        """
        Legacy nanoGPT-style MFU calculation (Version 1).
        
        Simple implementation using the PaLM paper Appendix B formula:
        FLOPs per token ≈ 6N + 12LHQT
        
        This is the original nanoGPT approach - simple, pedagogical, but limited:
        - Assumes Multi-Head Attention (no GQA support)
        - Assumes standard 4× FFN expansion
        - Hardcoded A100 peak (can be overridden)
        ...
        Args:
            ...
            num_gpus: Number of GPUs (for scaling hardware peak)
            use_sparse_specs: If True, use sparse specs (nanoGPT doesn't have this)
        
        Returns:
            dict: Basic MFU metrics (enhanced to return dict for compatibility)
        """
        # Get parameter count (like nanoGPT)
        ...
        L = cfg.n_layer
        H = cfg.n_head
        Q = cfg.n_embd // cfg.n_head  # head dimension
        T = cfg.block_size  # sequence length
        ...
        # Hardware peak (A100 by default, like nanoGPT)
        # nanoGPT hardcodes: flops_promised = 312e12
        hardware_specs = {
            'A100': 312e12,
            'H100': 989e12,
            'H200': 1979e12,
            'B200': 2250e12 if not use_sparse_specs else 4500e12,
        }
        
        # Try to auto-detect GPU (nanoGPT doesn't do this)
        gpu_name = 'A100'
        ...
        flops_promised_per_gpu = hardware_specs.get(gpu_name, 312e12)
        flops_promised = flops_promised_per_gpu * num_gpus
        
        # Calculate MFU
        flops_achieved = flops_per_iter / dt
        mfu = flops_achieved / flops_promised
        
        # Enhanced return (nanoGPT just returns scalar mfu)
        # We return dict for compatibility with training script
        return {
            'mfu': mfu,
            'mfu_percent': mfu * 100,
            ...
            'tokens_per_sec': (T * fwdbwd_per_iter) / dt,
            'tokens_per_sec_per_gpu': (T * fwdbwd_per_iter) / dt / max(num_gpus, 1),
            'tokens_per_iter': T * fwdbwd_per_iter,
            'hardware_peak_flops': flops_promised,
            'hardware_peak_tflops': flops_promised / 1e12,
            'achieved_tflops': flops_achieved / 1e12,
            'gpu_name': gpu_name,
            'precision': 'bf16',  # Assumed
            'num_gpus': num_gpus,
            'sparse_mode': use_sparse_specs,
            'architecture': cfg.get_architecture_name(),
            'calculation_method': 'nanogpt_legacy_v1'  # Legacy method marker
        }
