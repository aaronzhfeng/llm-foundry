...
class ConfigurableGPT(nn.Module):
    ...
    def estimate_mfu_detailed(self, fwdbwd_per_iter, dt, device_type='cuda', num_gpus=1, use_sparse_specs=False):
        ...
        This implementation follows the methodology from "The MFU Audit: Rectifying Model 
        FLOPs Utilization Methodology for Next-Generation Architectures" and addresses:
        
        1. Grouped Query Attention (GQA) correction (Section 4.1)
        2. SwiGLU FFN with dynamic intermediate_size (Section 3.2)
        3. Vocabulary/Logit layer overhead (Section 6.2)
        4. Dense vs Sparse hardware peaks (Section 5.1)
        ...
        Args:
            ...
            use_sparse_specs: If True, use sparse tensor core specs (2:4 sparsity) for B200/A6000.
        ...
        # ===== ATTENTION FLOPs (GQA-aware, per audit Section 4.1) =====
        # GQA Projections: Q(h->h) + K(h->h/G) + V(h->h/G) + O(h->h)
        # Total: 2*S*H*H * (2 + 2/G)
        attention_qkv_flops = 2 * S * H * H * (2 + 2/G)
        
        # Attention scores: 2 * S * S * H (QK^T for all heads)
        # Note: H is total embedding dim; attention is 2*S^2*H total (not a*S^2*H)
        attention_scores_flops = 2 * S * S * H
        
        # Attention output: 2 * S * S * H (softmax(QK^T) @ V for all heads)
        ...
        # ===== VOCABULARY/LOGIT LAYER FLOPs (Critical for small models, per audit Section 6.2) =====
        # Final LM head projection: H × V
        V = cfg.vocab_size
        logit_flops = 2 * S * H * V
        ...
        return {
            ...
            'calculation_method': 'combined_formula_v2'  # Combined algebraic formula
        }
