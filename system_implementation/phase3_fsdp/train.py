...
optimizer = model.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type) if not (ddp and use_fsdp) else model.module.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type) if hasattr(model, 'module') else model.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)

# For FSDP, we need to get the unwrapped model for configure_optimizers
if ddp and use_fsdp:
    ...
    # DDP or single GPU
    optimizer = model.module.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type) if ddp and not use_fsdp else model.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
